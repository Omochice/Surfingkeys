 ...
 * in direction `dir` (+1 forward, -1 backward), clamped to the string bounds.
 */
...
/**
 * Delete the word adjacent to `cur` in direction `dir`. Returns the resulting
 * string and the cursor offset after the deletion.
 */
...
function createInsert(): InsertMode {
    ...
    self.enableEmojiInsertion = () => {
        self.mappings!.add(":", {
            ...
            code: () => {
                setTimeout(() => {
                    emojiPrompt.activate(
                        getRealEdit(),
                        undefined,
                        runtime.conf.startToShowEmoji,
                        -1,
                    );
                }, 100);
            },
            ...
            Mode.handleMapKey.call(self, event, (last) => {
                ...
                if (pw) {
                    ...
                    if (str !== undefined && pos !== undefined) {
                        elm.value =
                            str.substr(0, elm.selectionStart) + pw + str.substr(elm.selectionEnd);
                        pos += pw.length;
