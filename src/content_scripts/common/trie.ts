 ...
 * `word` is filled in by {@link Trie.add}, so callers omit it.
 */
...
/**
 * A prefix tree mapping keystroke sequences to their metadata, used for key
 * binding lookup. Children are keyed by single characters; a node carries its
 * own `stem` character and, when it terminates a bound sequence, its `meta`.
 *
 * The implementation is intentionally faithful to the original prototype-based
 * version: matched-prefix reconstruction and branch pruning behave identically.
 */
export default class Trie {
    ...
    /**
     * Remove `word` and prune ancestor nodes that become empty as a result.
     * Returns the removed terminal node, or `undefined` if `word` was absent.
     */
    ...
    /**
     * Reconstruct the keystroke prefix matched up to this node. All words
     * completing from here share the same prefix, so any descent yields the
     * same result: the first `depth` characters of a completing word.
     */
