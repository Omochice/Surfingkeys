...
function applyUserSettings(delta: { error: string; settings: Record<string, unknown> }): void {
    if (delta.error !== "") {
        if (window === top) {
            ...
            console.log(
                "[SurfingKeys] Error found in settings({0}): {1}".format(
...
function getLargeElements(minWidth = 0.3, minHeight = 0.3): HTMLElement[] {
    ...
    const elements = getVisibleElements((element, visibleElements) => {
        ...
        if (
            Math.abs(viewportWidth - rect.width) < 4 &&
...
const flashElem = createElementWithContent("div", "", {
    style: "position: fixed; box-shadow: 0px 0px 4px 2px #63b2ff; background: transparent; z-index: 2140000000",
});
    ...
    if (pressedKeys.length > 0) {
        for (const hint of hints) {
            ...
            if (pressedKeys === label) {
                ...
                setSanitizedContent(
                    hint,
                    `<span style="opacity: 0.2;">${pressedKeys}</span>` +
                        label.substr(pressedKeys.length),
                );
                result.candidates++;
            } else {
