...
/**
 * Drives a successful `initGist` so the shared Gist closure holds a non-empty `gist`, a
...
// gist is still "".  They are placed immediately after the initGist describe
// blocks (which prime gist) and before any further primeGist calls.
// Instead we verify the behaviour via the initGist failure path: after a
// failed initGist the gist stays "". We also re-check the first bootDispatch
...
// We cover the gist=="" guard arm separately in the next describe block.
// ---------------------------------------------------------------------------
...
// The gist=="" guard is an alternative entry; the test below fires it after
// explicitly forcing a failed initGist so the singleton gist remains empty.
...
describe("start — readComment / editComment when gist initialisation failed", () => {
  it("readComment settles with status 1 when the gist init failed (gist stays empty)", async () => {
    // Force gist to stay "" by having initGist fail
    mockRequest.mockResolvedValue(gistsFail());
    const dispatch = bootDispatch();
    ...
    // gist is still "" → readComment should bail immediately
    mockRequest.mockReset();
...
// ---------------------------------------------------------------------------
// readComment — comment already in cached list (nr < comments.length)
// ---------------------------------------------------------------------------
...
describe("start — readComment reads from cached comment list", () => {
  it("reads directly from the cached list when the index is within range", async () => {
    ...
    // Populate comments cache by listing first
...
describe("start — readComment malformed per-comment JSON", () => {
  it("settles with status 1 when the fetched comment body is malformed JSON", async () => {
    ...
    // The Gist closure is a module singleton, so its comments cache leaks across
  ...
  it("settles with status 1 when the fetched comment lacks a body", async () => {
    ...
    // Prime the singleton's comments cache with a single known entry (see the
