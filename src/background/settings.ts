...
/**
 * Sends a (possibly deferred) response for a handled message; injected from the
 * composition root so the unit shares the one pending-port bookkeeping.
 */
...
/**
 * Projects `set` to the requested `keys`. A null/undefined/"" key set returns
 * the whole object; a single key or an array of keys returns just that subset.
 */
...
/**
 * Persists settings into a `chrome.storage` area. Sync storage has a quota, so
 * snippets loaded from a `localPath` are never written there; local storage
 * instead re-fetches and caches the snippets from that path before saving.
 */
...
/**
 * Dependencies the settings subsystem cannot own: the deferred-responder and
 * per-browser glue from the composition root, the shared mutable `conf` (also
 * read by the tab handlers), and the tab-core primitives a few settings actions
 * reach into — `sendTabMessage` to broadcast updates, plus `tabMessages`,
 * `setScrollPos`, the shared `handlers` registry, `newTabUrl` and `quit` for
 * the marks/session actions that drive tab navigation.
 */
...
/**
 * What the composition root needs back from the settings subsystem: the handler
 * map to register, plus the three infra functions still called by misc handlers
 * that stay in start.ts (`removeURL`'s mark-deletion branch and `localData`).
 */
...
/**
 * Settings subsystem: load/save/sync of settings, the blocklist/mouse-query
 * state toggles, the enable/disable state computation, VIM marks, and sessions.
 * Owns the settings storage logic and the user-script registration; takes its
 * cross-concern dependencies by injection so it never imports the tab core back.
 */
