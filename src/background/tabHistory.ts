/**
 * Most-recently-used tab navigation ring. Tracks the order tabs were activated
 * in (capped at the last 10) plus a cursor for stepping back and forth through
 * that order, so `historyTab`/`goToLastTab` can walk it. A `programmaticSwitch`
 * flag lets the unit ignore the `onActivated` event that its own navigation
 * triggers, so stepping through history does not itself rewrite the ring.
 *
 * Owns no chrome state and performs no I/O; the background listeners feed it
 * activations/removals and act on the tab id it returns.
 */
export interface TabHistory {
    ...
    /**
     * Move the cursor and return the tab to activate: `index` jumps to an
     * absolute (wrapping) position, otherwise step backward/forward clamped to
     * the ends. Returns undefined when the ring is empty.
     */
        ...
        navigate(message: { index?: any; backward?: boolean }): number | undefined {
            if (tabHistory.length > 0) {
                ...
                if (Object.prototype.hasOwnProperty.call(message, "index")) {
                    tabHistoryIndex =
                        (parseInt(message.index) + tabHistory.length) % tabHistory.length;
                } else {
