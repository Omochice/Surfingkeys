/**
 * Fetches a URL and decodes the body using the charset advertised in its
 * `content-type` header (falling back to UTF-8). Shared by the settings storage
 * (snippet loading), the `request` message handler, and the Gist closure, so it
 * lives in its own module rather than inside any one concern.
 *
