...
/**
 * Dependencies the tab core takes from the composition root: the deferred
 * responder, the shared mutable `conf` (written by settings, read here), the
 * per-browser glue, and the shared `handlers` registry for the two intra-tab
 * cross-calls (viewSource → openLink, closeTab → historyTab).
 */
...
/**
 * What the composition root needs back from the tab core: the handler map to
 * register, plus the primitives other units depend on — `sendTabMessage` and
 * `tabMessages`/`setScrollPos`/`newTabUrl` injected into settings, and
 * `filterByTitleOrUrl` injected into the history unit.
 */
...
/**
 * Tab and window core: the MRU/index/url bookkeeping maps and the tab lifecycle
 * listeners, the navigation/close/move helpers, and every tab- and
 * window-oriented message handler. Owns the queued-URL list because the
 * onRemoved handler drains it. Reads the shared `conf` by reference and reaches
 * the rest of the registry through the injected `handlers` for two cross-calls.
 */
    ...
    const handlersMap: Record<string, MessageHandler> = {
        ...
        moveTab: (message: any, sender: any, _sendResponse: any) => {
            chrome.tabs.query(
                ...
                (tabs: any[]) => {
                    const to = _fixTo(
                        sender.tab.index + message.step * message.repeats,
                        tabs.length,
                    );
